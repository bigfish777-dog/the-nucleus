...
// ─── Alerts ──────────────────────────────────────────────────────────────────
const overdueStage = leads.filter(l => l.stage === 'booked' && l.call_datetime && new Date(l.call_datetime) < now)
const staleProposals = leads.filter(l => l.stage === 'proposal_sent' && l.proposal_sent_at && (now.getTime() - new Date(l.proposal_sent_at).getTime()) > 7 * 86400000)
const noContact = leads.filter(l => l.last_contact_at && (now.getTime() - new Date(l.last_contact_at).getTime()) > 14 * 86400000 && !['closed_won','closed_lost','abandoned'].includes(l.stage))
const upcomingSecondCalls = leads.filter(l => l.stage === 'second_call_booked' && l.second_call_datetime && new Date(l.second_call_datetime) >= now)
